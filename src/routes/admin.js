...
router.get("/questionnaires", async (req, res) => {
  try {
    ...
    res.status(error.code === "DB_NOT_CONFIGURED" ? 503 : 500).json({ error: error.message || "Failed to load questionnaires" });
  }
});
...
router.get("/questionnaires/:id", async (req, res) => {
  try {
    ...
    res.status(error.code === "DB_NOT_CONFIGURED" ? 503 : 500).json({ error: error.message || "Failed to load questionnaire details" });
  }
});
...
router.patch("/questionnaires/:id/status", async (req, res) => {
  try {
    ...
    res.status(error.code === "DB_NOT_CONFIGURED" ? 503 : 500).json({ error: error.message || "Failed to update questionnaire status" });
  }
});
